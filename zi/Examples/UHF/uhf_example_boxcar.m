...
if isempty(strfind(options, 'BOX'))
    error('This example can only be ran on UHF Instruments with the BOX option enabled, device `%s` reports options `%s`.', device, options);
end

if ziDAQ('getConnectionAPILevel') < 4
    error('ziDAQServer is not using API Level >=4. This is required in order to obtain boxcar data with timestamps.');
end
