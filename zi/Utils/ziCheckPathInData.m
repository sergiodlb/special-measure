...
% $Id$
  ...
  path = lower(path);
  validCharacters = isstrprop(path, 'alphanum') | path == '/';
...
% Split the path into parts.
  ...
  % In general, we could be using API4, in which case polling int and double
  % nodes return a struct with fields 'value' and 'timestamp'.
